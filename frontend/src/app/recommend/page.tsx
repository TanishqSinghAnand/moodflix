...
export default function RecommendPage() {
  ...
  // Fetch recommendations when session ID is available
  useEffect(() => {
  ...
  const handleFeedback = async (title: RecommendedTitle, relevant: boolean) => {
    ...
    toast(relevant ? "👍 Thanks!" : "👎 Noted — we'll improve!", { icon: "🎯" });
  };
      ...
      <div className="max-w-5xl mx-auto w-full px-4 py-8">
        ...
        {data.serendipity_pick && (
          ...
          >
            ...
            <MediaCard
              title={data.serendipity_pick}
              sessionId={data.session_id}
              feedback={feedback[data.serendipity_pick.id]}
              onFeedback={(r) => handleFeedback(data.serendipity_pick!, r)}
              featured
            />
          </motion.div>
        )}
            ...
            {data.results.map((title, i) => (
              ...
              >
                <MediaCard
                  title={title}
                  sessionId={data.session_id}
                  feedback={feedback[title.id]}
                  onFeedback={(r) => handleFeedback(title, r)}
                />
              </motion.div>
            ))}
          </AnimatePresence>
